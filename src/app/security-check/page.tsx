...
export default function SecurityCheckPage() {
  ...
  if (!done) {
    ...
    return (<>
      <section style={{ padding: '120px clamp(20px,4vw,52px) 48px', background: 'linear-gradient(135deg, #0B1220 0%, #1A2744 100%)', color: '#fff', textAlign: 'center' , position: 'relative', overflow: 'hidden'}}>
        ...
        <h1 style={{ fontFamily: "'Pretendard'", fontWeight: 900, fontSize: 'clamp(28px,4.5vw,48px)', lineHeight: 1.2, letterSpacing: '-.03em', marginBottom: 12 }}>우리 회사 IT 보안,<br/>몇 점일까요?</h1>
        <p style={{ fontFamily: "'Pretendard'", fontWeight: 400, fontSize: 18, opacity: .7 }}>10개 질문에 답하면 보안 점수와 취약 영역을 알려드립니다.</p>
      </section>
  ...
  return (<>
    <section style={{ padding: '120px clamp(20px,4vw,52px) 48px', background: 'linear-gradient(135deg, #0B1220 0%, #1A2744 100%)', color: '#fff', textAlign: 'center' , position: 'relative', overflow: 'hidden'}}>
      ...
      <h1 style={{ fontFamily: "'Pretendard'", fontWeight: 900, fontSize: 'clamp(28px,4.5vw,48px)', lineHeight: 1.2, letterSpacing: '-.03em' }}>보안 점검 결과</h1>
    </section>
