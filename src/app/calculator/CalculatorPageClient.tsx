...
const DEFAULT_HERO = {
  eyebrow: 'IT COST CALCULATOR',
  title: '우리 회사 IT 비용,\n1분 만에 계산하세요',
  desc: '직원 수와 필요한 솔루션만 선택하면 예상 비용을 바로 확인할 수 있습니다.',
};

const DEFAULT_CTA = {
  discount_msg: '유니온시스템즈를 통하면 평균 15% 절감',
  reset_btn: '다시 계산하기',
...
const DEFAULTS = {
  calculator_hero: DEFAULT_HERO,
  calculator_cta: DEFAULT_CTA,
  calculator_ui: DEFAULT_UI,
} as const;
...
export default function CalculatorPageClient({ ssrContent }: { ssrContent: Record<string, string> }) {
  ...
  const [m365Plan, setM365Plan] = useState(1);
  ...
  return (<>
    ...
    <section style={{ padding: '120px clamp(20px,4vw,52px) 48px', background: 'linear-gradient(135deg, #111214 0%, #111214 100%)', color: '#fff', textAlign: 'center' , position: 'relative', overflow: 'hidden'}}>
      ...
      <p style={{ fontFamily: "'Pretendard'", fontWeight: 600, fontSize: 18, letterSpacing: '.14em', opacity: .7, marginBottom: 12 }}>
        <E id="calculator_hero.eyebrow" editMode={editMode}>{content.calculator_hero.eyebrow}</E>
      </p>
      <h1 style={{ fontFamily: "'Pretendard'", fontWeight: 900, fontSize: 'clamp(28px,4.5vw,48px)', lineHeight: 1.2, letterSpacing: '-.03em', marginBottom: 16, whiteSpace: 'pre-line' }}>
        <E id="calculator_hero.title" editMode={editMode}>{content.calculator_hero.title}</E>
      </h1>
      <p style={{ fontFamily: "'Pretendard'", fontWeight: 400, fontSize: 18, opacity: .85, maxWidth: 500, margin: '0 auto' }}>
        <E id="calculator_hero.desc" editMode={editMode}>{content.calculator_hero.desc}</E>
      </p>
    </section>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        {step === 1 && (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 0, padding: 'clamp(24px,4vw,40px)' }}>
            <h2 style={{ fontFamily: "'Pretendard'", fontWeight: 800, fontSize: 22, color: 'var(--ink)', marginBottom: 32 }}><E id="calculator_ui.step1_title" editMode={editMode}>{content.calculator_ui.step1_title}</E></h2>

            <label style={{ fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, color: 'var(--ink)', display: 'block', marginBottom: 12 }}>
              <E id="calculator_ui.step1_emp_label" editMode={editMode}>{content.calculator_ui.step1_emp_label}</E> <span style={{ color: 'var(--accent)', fontSize: 24, fontWeight: 900 }}>{employees}</span>명
            </label>
            ...
            <label style={{ fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, color: 'var(--ink)', display: 'block', marginBottom: 12 }}><E id="calculator_ui.step1_industry" editMode={editMode}>{content.calculator_ui.step1_industry}</E></label>
            ...
            <button onClick={() => { if (industry) setStep(2); }}
              ...
              <E id="calculator_ui.step1_next" editMode={editMode}>{content.calculator_ui.step1_next}</E>
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 0, padding: 'clamp(24px,4vw,40px)' }}>
            <h2 style={{ fontFamily: "'Pretendard'", fontWeight: 800, fontSize: 22, color: 'var(--ink)', marginBottom: 8 }}><E id="calculator_ui.step2_title" editMode={editMode}>{content.calculator_ui.step2_title}</E></h2>
            <p style={{ fontFamily: "'Pretendard'", fontWeight: 400, fontSize: 18, color: 'var(--ink2)', marginBottom: 32 }}><E id="calculator_ui.step2_hint" editMode={editMode}>{content.calculator_ui.step2_hint}</E></p>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
              <button onClick={() => setStep(1)}
                style={{ flex: 1, padding: '14px 0', borderRadius: 0, border: '1px solid var(--line)', background: 'transparent', color: 'var(--ink2)', fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, cursor: 'pointer' }}><E id="calculator_ui.step2_prev" editMode={editMode}>{content.calculator_ui.step2_prev}</E></button>
              <button onClick={() => { if (Object.values(selected).some(v => v)) setStep(3); }}
                disabled={!Object.values(selected).some(v => v)}
                style={{ flex: 2, padding: '14px 0', borderRadius: 0, background: Object.values(selected).some(v => v) ? 'var(--accent)' : 'var(--line)', color: '#fff', fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, border: 'none', cursor: Object.values(selected).some(v => v) ? 'pointer' : 'not-allowed' }}><E id="calculator_ui.step2_calc" editMode={editMode}>{content.calculator_ui.step2_calc}</E></button>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ background: 'linear-gradient(135deg, #111214, #111214)', borderRadius: 0, padding: 'clamp(24px,4vw,40px)', color: '#fff', textAlign: 'center', marginBottom: 24 }}>
              ...
              <div style={{ marginTop: 24, padding: '16px 20px', borderRadius: 14, background: 'rgba(255,255,255,.15)', backdropFilter: 'blur(8px)' }}>
                <p style={{ fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18 }}>
                  <E id="calculator_cta.discount_msg" editMode={editMode}>{content.calculator_cta.discount_msg}</E>
                </p>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 0, overflow: 'hidden', marginBottom: 24 }}>
              <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--line)' }}>
                <h3 style={{ fontFamily: "'Pretendard'", fontWeight: 800, fontSize: 18, color: 'var(--ink)' }}><E id="calculator_ui.result_detail_title" editMode={editMode}>{content.calculator_ui.result_detail_title}</E></h3>
              </div>
              {results.map((r, i) => {
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 0, padding: 'clamp(24px,4vw,40px)', marginBottom: 24, textAlign: 'center' }}>
              <h3 style={{ fontFamily: "'Pretendard'", fontWeight: 800, fontSize: 18, color: 'var(--ink)', marginBottom: 24 }}><E id="calculator_ui.result_chart_title" editMode={editMode}>{content.calculator_ui.result_chart_title}</E></h3>
              <div style={{ position: 'relative', width: 200, height: 200, margin: '0 auto' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{ fontFamily: "'Pretendard'", fontWeight: 500, fontSize: 18, color: 'var(--ink2)' }}><E id="calculator_ui.result_chart_unit" editMode={editMode}>{content.calculator_ui.result_chart_unit}</E></span>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 12 }} className="calc-cta">
              <button onClick={() => { setStep(1); setSelected({}); }}
                style={{ flex: 1, padding: '14px 0', borderRadius: 0, border: '1px solid var(--line)', background: 'transparent', color: 'var(--ink2)', fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, cursor: 'pointer' }}>
                <E id="calculator_cta.reset_btn" editMode={editMode}>{content.calculator_cta.reset_btn}</E>
              </button>
              <Link href="/contact"
                style={{ flex: 2, padding: '14px 0', borderRadius: 0, background: 'var(--accent)', color: '#fff', fontFamily: "'Pretendard'", fontWeight: 700, fontSize: 18, textDecoration: 'none', textAlign: 'center', display: 'block' }}>
                <E id="calculator_cta.contact_btn" editMode={editMode}>{content.calculator_cta.contact_btn}</E>
              </Link>
            </div>
          </div>
        )}
