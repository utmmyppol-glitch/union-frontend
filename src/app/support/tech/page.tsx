    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div className="wrap" style={{
          ...
          padding: '120px clamp(20px,4vw,52px) 64px',
            ...
            <h1 style={{
              ...
              유니온시스템즈의<br />
              <span style={{ color: 'var(--accent)' }}>리뉴얼</span>, <span style={{ color: 'var(--accent)' }}>기술지원팀</span>을 소개합니다.
            </h1>
        </div>
      </section>
          ...
          <div className="tech-svc" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {SERVICES.map((s, i) => (
              <div key={s.title} className="reveal" style={{
                ...
                <h3 style={{ fontWeight: 800, fontSize: 18, color: 'var(--ink)', margin: '0 0 8px' }}>{s.title}</h3>
                <p style={{ fontSize: 18, color: 'var(--ink2)', margin: 0, lineHeight: 1.6, whiteSpace: 'pre-line' }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="tech-process" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {SUPPORT_ITEMS.map((s, i) => (
              <div key={s.num} className="reveal tech-proc-card" style={{
                ...
                <h3 style={{ fontWeight: 700, fontSize: 18, color: 'var(--ink)', margin: '8px 0 6px' }}>{s.title}</h3>
                <p style={{ fontSize: 18, lineHeight: 1.6, color: 'var(--ink2)', margin: 0 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 1 }}>
          <div className="reveal" style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', fontSize: 18, color: 'var(--ink2)' }}>
              <span><strong style={{ color: 'var(--ink)' }}>전화 문의</strong> 02-706-8999</span>
              <span><strong style={{ color: 'var(--ink)' }}>운영시간</strong> 평일 10 – 18시 (점심시간 12:00 – 13:00 제외, 주말 및 공휴일 휴무)</span>
            </div>
          </div>
              ...
              <tbody>
                {TECH_TEAM.map((m, i) => (
                  <tr key={m.email} style={{ borderBottom: '1px solid var(--line)' }}>
                    {i === 0 && (
                      <td rowSpan={TECH_TEAM.length} style={{
                    ...
                    <td style={{ padding: '22px 28px' }}>
                      <a href={`mailto:${m.email}`} style={{ color: 'var(--ink2)', textDecoration: 'none', fontSize: 18 }}>{m.email}</a>
                    </td>
                  </tr>
                ))}
                {DA_TEAM.map((m, i) => (
                  <tr key={m.email} style={{ borderBottom: i < DA_TEAM.length - 1 ? '1px solid var(--line)' : 'none' }}>
                    {i === 0 && (
                      <td rowSpan={DA_TEAM.length} style={{
                        padding: '28px 36px', fontWeight: 800, fontSize: 18, color: 'var(--ink)',
                        borderRight: '1px solid var(--line)', verticalAlign: 'middle',
              ...
              <div key={idx} style={{ border: '1px solid rgba(255,255,255,.08)', overflow: 'hidden' }}>
                <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 18, fontWeight: 600, color: '#fff' }}>{faq.question}</span>
                  </div>
                ...
                {openFaq === idx && (
                  <div style={{ padding: '0 20px 16px', borderTop: '1px solid rgba(255,255,255,.06)' }}>
                    <p style={{ fontSize: 18, lineHeight: 1.7, color: 'rgba(255,255,255,.5)', margin: '12px 0 0' }}>{faq.answer}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      <section style={{ padding: 'clamp(56px, 8vw, 96px) 0', background: 'var(--soft)', borderTop: '1px solid var(--line)' , position: 'relative', overflow: 'hidden'}}>
        ...
        <div className="wrap" style={{ textAlign: 'center' }}>
          <h2 className="reveal" style={{ fontWeight: 900, fontSize: 'clamp(26px, 3.5vw, 40px)', color: 'var(--ink)', margin: '0 0 16px' }}>기술지원이 필요하신가요?</h2>
